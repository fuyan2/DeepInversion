...
def _resnet(arch, block, layers, pretrained, progress, device,num_classes=10, **kwargs):
    model = ResNet(block, layers, num_classes=num_classes, **kwargs)
    if pretrained:
        script_dir = os.path.dirname(__file__)
        state_dict = torch.load(script_dir + '/models/'+arch+'.pt', map_location=device)
        model.load_state_dict(state_dict)
    return model


def resnet18(pretrained=False, progress=True, num_classes=10, device='cpu', **kwargs):
...
def resnet34(pretrained=False, progress=True, num_classes=10, device='cpu', **kwargs):
...
def resnet152(pretrained=False, progress=True, num_classes=10, device='cpu', **kwargs):
